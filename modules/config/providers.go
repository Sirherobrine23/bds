...
// Config represents a config provider
...
var (
	_ Config        = (*iniConfigProvider)(nil)
...
func NewConfigProviderFromData(configContent string) (Config, error) {
...
func NewConfigProviderFromFile(file string) (Config, error) {
...
func (p *iniConfigProvider) Save() error {
	...
	if fi.Mode().Perm() > 0o600 {
		if err = os.Chmod(filename, 0o600); err != nil {
			log.Error("Failed changing conf file permissions to -rw-------. Consider changing them manually.")
		}
	}
	return nil
}
...
func (p *iniConfigProvider) PrepareSaving() (Config, error) {
	if p.file == "" {
...
func mustMapSetting(rootCfg Config, sectionName string, setting any) {
	if err := rootCfg.Section(sectionName).MapTo(setting); err != nil {
		data := fmt.Sprintf("Failed to map %s settings: %v", sectionName, err)
		log.Error(data)
		panic(data)
	}
}
...
func NewConfigProviderForLocale(source any, others ...any) (Config, error) {
